...
const WaterBillComponent = React.forwardRef<HTMLDivElement, WaterBillProps>(({ data, price, t, i18n }, ref) => {
    ...
    >
      ...
      <div style={{ color: '#000', fontSize: 16, fontWeight: 500 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto auto', columnGap: 24 }}>
          ...
          <p style={{ marginTop: 5 }}>
            <span style={{ color: '#2563eb' }}>{t('previous_meter')}</span>: {data.pre_water_number}
          </p>
          <p style={{ marginTop: 5 }}>
            <span style={{ color: '#2563eb' }}>{t('current_meter')}</span>: {data.current_water_number}
          </p>
        </div>
        <table style={{ width: '100%', border: '1px solid #ccc', marginTop: 16 }}>
          ...
          <tbody>
            <tr>
              <td style={{ padding: 8, border: '1px solid #ccc', width: '50%' }}>{data.consumption} m³</td>
...
export default WaterBillComponent
